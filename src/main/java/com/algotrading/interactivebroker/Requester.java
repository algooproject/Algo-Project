...
public class Requester {
	 ...
	 * "delayed-frozen" market data. Requires TWS/IBG v963+. The API can receive
	 * frozen market data from Trader Workstation. Frozen market data is the last
	 * data recorded in our system. During normal trading hours, the API receives
	 * real-time market data. Invoking this function with argument 2 requests a
	 * switch to frozen data immediately or after the close. When the market reopens
	 * the next data the market data type will automatically switch back to real
	 * time if available.
	 * 
	 * @param marketDataType by default only real-time (1) market data is enabled
	 *                       sending 1 (real-time) disables frozen, delayed and
	 *                       delayed-frozen market data sending 2 (frozen) enables
	 *                       frozen market data sending 3 (delayed) enables delayed
	 *                       and disables delayed-frozen market data sending 4
	 *                       (delayed-frozen) enables delayed and delayed-frozen
	 *                       market data
	 */
	 ...
	 * 
	 * Requests real time market data. Returns market data for an instrument either
	 * in real time or 10-15 minutes delayed (depending on the market data type
	 * specified)
	 * 
	 ...
	 *                          <li>101 Option Open Interest (currently for
	 *                          stocks)</li>
	 *                          <li>104 Historical Volatility (currently for
	 *                          stocks)</li>
	 *                          <li>106 Option Implied Volatility (currently for
	 *                          stocks)</li>
	 *                          <li>162 Index Future Premium</li>
	 *                          <li>165 Miscellaneous Stats</li>
	 *                          <li>221 Mark Price (used in TWS P&L
	 *                          computations)</li>
	 *                          <li>225 Auction values (volume, price and
	 *                          imbalance)</li>
	 *                          <li>233 RTVolume - contains the last trade price,
	 *                          last trade size, last trade time, total volume,
	 *                          VWAP, and single trade flag.</li>
	 *                          <li>236 Shortable</li>
	 *                          <li>256 Inventory</li>
	 *                          <li>258 Fundamental Ratios</li>
	 *                          <li>411 Realtime Historical Volatility</li>
	 *                          <li>456 IBDividends</li>
	 * @param snapshot          for users with corresponding real time market data
	 *                          subscriptions. A true value will return a one-time
	 *                          snapshot, while a false value will provide streaming
	 *                          data.
	 * @param regulatory        snapshot for US stocks requests NBBO snapshots for
	 *                          users which have "US Securities Snapshot Bundle"
	 *                          subscription but not corresponding Network A, B, or
	 *                          C subscription necessary for streaming * market
	 *                          data. One-time snapshot of current market price that
	 *                          will incur a fee of 1 cent to the account per
	 *                          snapshot.
	 * @param marketDataOptions unknown
	 */
	 ...
	 * @param startDateTime i.e. "20170701 12:01:00". Uses TWS timezone specified at
	 *                      login.
	 * @param endDateTime   i.e. "20170701 13:01:00". In TWS timezone. Exactly one
	 *                      of start time and end time has to be defined.
	 * @param numberOfTicks Number of distinct data points. Max currently 1000 per
	 *                      request.
	 * @param whatToShow    (Bid_Ask, Midpoint, Trades) Type of data requested.
	 * @param useRth        Data from regular trading hours (1), or all available
	 *                      hours (0)
	 * @param ignoreSize    A filter only used when the source price is Bid_Ask
	 * @param miscOptions   should be defined as null, reserved for internal use
	 */
	...
	/**
	 * Subscribes to an specific account's information and portfolio Through this
	 * method, a single account's subscription can be started/stopped. As a result
	 * from the subscription, the account's information, portfolio and last update
	 * time will be received at EWrapper::updateAccountValue,
	 * EWrapper::updateAccountPortfolio, EWrapper::updateAccountTime respectively.
	 * All account values and positions will be returned initially, and then there
	 * will only be updates when there is a change in a position, or to an account
	 * value every 3 minutes if it has changed. Only one account can be subscribed
	 * at a time. A second subscription request for another account when the
	 * previous one is still active will cause the first one to be canceled in
	 * favour of the second one. Consider user reqPositions if you want to retrieve
	 * all your accounts' portfolios directly.
	 * 
	 * @param subscribe set to true to start the subscription and to false to stop
	 *                  it.
	 * @param acctCode  the account id (i.e. U123456) for which the information is
	 *                  requested.
	 */
	 ...
	 * 
	 * @return three days of data counting backwards from July 1st 2013 at 23:59:59
	 *         GMT resulting in all the available bars of the last three days until
	 *         the date and time specified. It is possible to specify a timezone
	 *         optionally. The resulting bars will be returned in
	 *         EWrapper::historicalData
	 * 
	 * @param tickerId the request's unique identifier. contract the contract for
	 *                 which we want to retrieve the data. endDateTime request's
	 *                 ending time with format yyyyMMdd HH:mm:ss {TMZ}
	 *                 durationString the amount of time for which the data needs to
	 *                 be retrieved: " S (seconds) - " D (days) " W (weeks) - " M
	 *                 (months) " Y (years) barSizeSetting the size of the bar: 1
	 *                 sec 5 secs 15 secs 30 secs 1 min 2 mins 3 mins 5 mins 15 mins
	 *                 30 mins 1 hour 1 day whatToShow the kind of information being
	 *                 retrieved: TRADES MIDPOINT BID ASK BID_ASK
	 *                 HISTORICAL_VOLATILITY OPTION_IMPLIED_VOLATILITY FEE_RATE
	 *                 REBATE_RATE useRTH set to 0 to obtain the data which was also
	 *                 generated outside of the Regular Trading Hours, set to 1 to
	 *                 obtain only the RTH data formatDate set to 1 to obtain the
	 *                 bars' time as yyyyMMdd HH:mm:ss, set to 2 to obtain it like
	 *                 system time format in seconds keepUpToDate set to True to
	 *                 received continuous updates on most recent bar data. If True,
	 *                 and endDateTime cannot be specified.
	 *
	 ...
	 * 
	 * @parm requestId conId - contract id of ticker providerCodes - a '+'-separated
	 *       list of provider codes startDateTime - marks the (exclusive) start of
	 *       the date range. The format is yyyy-MM-dd HH:mm:ss.0 endDateTime - marks
	 *       the (inclusive) end of the date range. The format is yyyy-MM-dd
	 *       HH:mm:ss.0 totalResults - the maximum number of headlines to fetch (1 -
	 *       300) historicalNewsOptions reserved for internal use. Should be defined
	 *       as null.
	 *
	 ...
	 * Requests historical Time&Sales data for an instrument.
	 * 
	 * @param reqId id of the request contract Contract object that is subject of
	 *              query startDateTime,i.e. "20170701 12:01:00". Uses TWS timezone
	 *              specified at login. endDateTime,i.e. "20170701 13:01:00". In TWS
	 *              timezone. Exactly one of start time and end time has to be
	 *              defined. numberOfTicks Number of distinct data points. Max
	 *              currently 1000 per request. whatToShow (Bid_Ask, Midpoint,
	 *              Trades) Type of data requested. useRth Data from regular trading
	 *              hours (1), or all available hours (0) ignoreSize A filter only
	 *              used when the source price is Bid_Ask miscOptions should be
	 *              defined as null, reserved for internal use
	 */
	 ...
	 * 
	 * @param numIds deprecated- this parameter will not affect the value returned
	 *               to nextValidId
	 * 
	 ...
	 * 
	 * @param tickerId the request's identifier contract the Contract for which the
	 ...
	 * 
	 * The market rule for an instrument on a particular exchange provides details
	 * about how the minimum price increment changes with price A list of market
	 * rule ids can be obtained by invoking reqContractDetails on a particular
	 * contract. The returned market rule ID list will provide the market rule ID
	 * for the instrument in the correspond valid exchange list in contractDetails.
	 * 
	 ...
	 * 
	 * @param reqId id to specify the request pattern - either start of ticker
	 ...
	 * 
	 * @param tickerId the request's identifier contract the Contract for which the
	 *                 data is being requested genericTickList comma separated ids
	 *                 of the available generic ticks: 100 Option Volume (currently
	 *                 for stocks) 101 Option Open Interest (currently for stocks)
	 *                 104 Historical Volatility (currently for stocks) 105 Average
	 *                 Option Volume (currently for stocks) 106 Option Implied
	 *                 Volatility (currently for stocks) 162 Index Future Premium
	 *                 165 Miscellaneous Stats 221 Mark Price (used in TWS P&L
	 *                 computations) 225 Auction values (volume, price and
	 *                 imbalance) 233 RTVolume - contains the last trade price, last
	 *                 trade size, last trade time, total volume, VWAP, and single
	 *                 trade flag. 236 Shortable 256 Inventory 258 Fundamental
	 *                 Ratios 411 Realtime Historical Volatility 456 IBDividends
	 *                 snapshot for users with corresponding real time market data
	 *                 subscriptions. A true value will return a one-time snapshot,
	 *                 while a false value will provide streaming data. regulatory
	 *                 snapshot for US stocks requests NBBO snapshots for users
	 *                 which have "US Securities Snapshot Bundle" subscription but
	 *                 not corresponding Network A, B, or C subscription necessary
	 *                 for streaming * market data. One-time snapshot of current
	 *                 market price that will incur a fee of 1 cent to the account
	 *                 per snapshot.
	 * 
	...
	/**
	 * @return the timestamp of earliest available historical data for a contract
	 *         and data type.
	 * 
	 * @param tickerId - an identifier for the request contract - contract object
	 *                 for which head timestamp is being requested whatToShow - type
	 *                 of data for head timestamp - "BID", "ASK", "TRADES", etc
	 *                 useRTH - use regular trading hours only, 1 for yes or 0 for
	 *                 no formatDate set to 1 to obtain the bars' time as yyyyMMdd
	 ...
	 * @param group set to "All" to return account summary data for all accounts, or
	 *              set to a specific Advisor Account Group name that has already
	 *              been created in TWS Global Configuration.
	 * @param tags  a comma separated list with the desired tags: AccountType ��
	 *              Identifies the IB account structure
	 *              <li>NetLiquidation �� The basis for determining the price of the
	 ...
	 *              dollar value of securities that one may purchase in a securities
	 *              account without depositing additional funds
	 *              <li>EquityWithLoanValue �� Forms the basis for determining
	 *              whether a client has the necessary assets to either initiate or
	 *              maintain security positions. Cash + stocks + bonds + mutual
	 *              funds
	 ...
	 *              <li>LookAheadMaintMarginReq �� Maintenance Margin requirement of
	 *              whole portfolio as of next period's margin change
	 ...
	 *              liquidation
	 *              <li>DayTradesRemaining �� The Number of Open/Close trades a user
	 ...
	 *              <li>$LEDGER �� Single flag to relay all cash balance tags*, only
	 *              in base currency.
	 ...
	 * @param acctCode  the account id (i.e. U123456) for which the information is
	 *                  requested.
	 * @see reqPositions, EWrapper::updateAccountValue, EWrapper::updatePortfolio,
	 ...
	 * 
	 * @param requestId    identifier to label the request
	 ...
	 * @param ledgerAndNLV returns light-weight request; only currency positions as
	 ...
	 * EWrapper:contractDetails. Though it is now (in API version > 9.72.12) advised
	 * to use reqSecDefOptParams for that purpose.
	 * 
	 * @param reqId    the unique request identifier.
	...
	/**
	 * Requests current day's (since midnight) executions matching the filter. Only
	 * the current day's executions can be retrieved. Along with the executions, the
	 * CommissionReport will also be returned. The execution details will arrive at
	 * EWrapper:execDetails.
	 * 
	 * @param reqId  the request's unique identifier.
	 * @param filter the filter criteria used to determine which execution reports
	 *               are returned.
	...
	/**
	 * Requests family codes for an account, for instance if it is a FA, IBroker, or
	 * associated account.
	 * 
	 ...
	 * 
	 * @param reqId      the request's unique identifier.
	 * @param contract   the contract's description for which the data will be
	 *                   returned.
	 ...
	 *                   <li>CalendarReport: Company calendar from Wall Street
	 *                   Horizons
	...
	/**
	 * Cancels all active orders. This method will cancel ALL open orders including
	 * those placed directly from TWS.
	 * 
	 * @see cancelOrder
	 */
	...
	/**
	 * Requests venues for which market data is returned to updateMktDepthL2 (those
	 * with market makers)
	 * 
	 ...
	 * @param newsArticleOptions - reserved for internal use. Should be defined as
	 *                           null.
	 ...
	 * 
	 * @param allMessages - if set to true, will return all the existing bulletins
	 *                    for the current day, set to false to receive only the new
	 *                    bulletins.
	...
	/**
	 * Requests all open orders places by this specific API client (identified by
	 * the API client id). For client ID 0, this will bind previous manual TWS
	 * orders.
	 * 
	 ...
	 * @param conId     - contract ID (conId) of contract to receive daily PnL
	 *                  updates for. Note: does not return message if invalid conId
	 *                  is entered
	 */
	 ...
	 * @param account   - If an account Id is provided, only the account's positions
	 ...
	 * request is subject to the same pacing as any historical data request: no more
	 * than 60 API queries in more than 600 seconds. Real time bars subscriptions
	 * are also included in the calculation of the number of Level 1 market data
	 ...
	 * @param whatToShow - the nature of the data being retrieved: TRADES, MIDPOINT,
	 *                   BID, ASK
	 * @param useRTH     set to 0 to obtain the data which was also generated
	 *                   ourside of the Regular Trading Hours, set to 1 to obtain
	 *                   only the RTH data
	...
	/**
	 * Requests an XML list of scanner parameters valid in TWS. Not all parameters
	 * are valid from API scanner.
	 * 
	 ...
	 * 
	 * @param reqId        - the request's identifier
	 * @param subscription - summary of the scanner subscription including its
	 *                     filters.
	 ...
	 * @param futFopExchange    - The exchange on which the returned options are
	 *                          trading. Can be set to the empty string "" for all
	 *                          exchanges.
	...
	/**
	 * Requests pre-defined Soft Dollar Tiers. This is only supported for registered
	 * professional advisors and hedge and mutual funds who have configured Soft
	 * Dollar Tiers in Account Management. Refer to:
	 ...
	 * @param tickType      - tick-by-tick data type: "Last", "AllLast", "BidAsk" or
	 *                      "MidPoint".
