...
public class Expr implements Node {
	...
	public void parse(Context context) throws ParseException {
		if (context.currentToken().equals("AND(")) {
			...
			node.parse(context);
		} else if (context.currentToken().equals("isReentryByRsiHigher[")) {
			node = new isReentryByRsiHigherThanInterpreter();
			node.parse(context);
		} else if (context.currentToken().equals("isStockEnabled[")) {
			node = new isStockEnabledInterpreter();
			node.parse(context);
		} else if (context.currentToken().equals("isExitByClosingDownTo[")) {
			node = new isExitByClosingDownToInterpreter();
			node.parse(context);
		} else {
