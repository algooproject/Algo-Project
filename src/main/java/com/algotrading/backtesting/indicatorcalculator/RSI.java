...
public class RSI implements IRsiCalculator {
	...
	public RSI(Map<Date, Double> datedprice, Date recent, int magnitude, int sma_magnitude) throws Exception {
		...
		List<Map<Date, Double>> plus_minus = differentiate(datedprice); // get
																		// the
																		// positive
																		// and
																		// negative
		...
		this.ema_plus = new EMA(this.plus, recent, magnitude, sma_magnitude); // create
																				// EMA
																				// by
																				// the
																				// positive
																				// part
		this.ema_minus = new EMA(this.minus, recent, magnitude, sma_magnitude); // create
																				// EMA
																				// by
																				// the
																				// negative
																				// part
		this.line = calLine(this.ema_plus.getLine(), this.ema_minus.getLine()); // get
																				// the
																				// RSI
	...
	private int magnitude, sma_magnitude; // E.g. 50 points of a 10-day MA. Then
											// magnitude = 10, linelen = 50
	private double value; // the EMA of the recent date
	private Map<Date, Double> line; // the EMA line, with the last element the
									// last SMA
	private Map<Date, Double> plus, minus; // the positive and negative
											// increment of dated price
	...
	public void setAlpha(double alpha) { // setting the alpha used in those
											// EMA's
		if (this.alpha != alpha) {
			this.alpha = alpha;
	...
	public void setAutoAlpha() { // setting the default alpha used in those
									// EMA's ( 2 / N)
	...
	public List<Map<Date, Double>> differentiate(Map<Date, Double> datedprice) { // the
																					// function
																					// which
																					// extracts
																					// the
	...
	private Map<Date, Double> calLine(Map<Date, Double> plus, Map<Date, Double> minus) { // get
																							// the
																							// line
																							// with
																							// the
	...
	public Map<Date, Double> calculate(Map<Date, Double> datedprice, int magnitude, int sma_magnitude)
			throws Exception { // get RSI, open to outside calls
		Map<Date, Double> line = new TreeMap<Date, Double>();
